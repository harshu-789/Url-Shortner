...
// ✅ Handle preflight for ALL routes (must be before other routes)
...
// ✅ Move /ping before the /:id catch-all route
app.get("/ping", (req, res) => {
  res.json({ message: "pong" });
});
...
app.use("/api/auth", authRouter);
app.use("/api/user", userRouter);
app.get("/:id", redirectShortUrl);  // Keep this LAST — it's a catch-all
app.use(errorHandler);
